...
export default function DealDetailPage() {
  ...
  const pullNews = async () => {
    ...
    setNewsNotes(null)
    try {
      const res = await fetch('/api/deals/news', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          company_name: deal.company_name,
          ...
          parent_portco: deal.parent_portco,
        }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed to pull news')
      setNews(data.articles || [])
      setNewsNotes(data.market_notes || null)
    } catch (e: any) {
    ...
    try {
      const res = await fetch('/api/deals/share-cim', {
        ...
        body: JSON.stringify({
          ...
          historical_financials: (deal as any).historical_financials,
          customer_concentration: (deal as any).customer_concentration,
          employee_count: (deal as any).employee_count,
          ...
          <div style={{ maxWidth: '900px' }}>
            ...
            <div className="card" style={{ padding: '20px', marginTop: '20px' }}>
              ...
              {!loadingSimilar && similarDeals.length === 0 && !similarError && (
                <div style={{ fontSize: '12px', color: 'var(--text-muted)', fontStyle: 'italic' }}>Click "Find similar" to discover deals with similar sector, size, and business profile.</div>
              )}
