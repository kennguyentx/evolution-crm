...
// Shared Nexus assistant engine: tool definitions, tool execution, system prompt,
// and a non-streaming agent loop. Used by both:
//   - app/api/assistant/route.ts       (web chat — streaming + write confirmation)
//   - app/api/notes/email/route.ts     ("Ask:" emails to intake@ — non-streaming, replies by email)

import Anthropic from '@anthropic-ai/sdk'
import { createClient } from '@supabase/supabase-js'
import { AI_MODELS } from '@/lib/ai-config'

export const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY, maxRetries: 4 })
...
export const TOOLS: any[] = [
  ...
  {
    name: 'create_deal',
    description: 'Create a new deal and log it into the pipeline. Use when the user mentions receiving a teaser, being introduced to a company, or wanting to track a new opportunity. Collect as much info as provided — at minimum a company name.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Update any field on a deal. First use search_deals to find the deal ID, then call this tool.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Update any field on a CRM contact. First use search_contacts to find the contact ID.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Update any field on a capital raise participant. First use get_capital_raises to find the participant ID.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'log_note',
    description: 'Log a meeting note or interaction against a deal, contact, or raise.',
    input_schema: {
      type: 'object',
      properties: {
...
export const WRITE_TOOLS = new Set(['create_deal', 'update_deal_field', 'update_contact_field', 'update_raise_participant', 'log_note', 'create_calendar_event'])
...
export async function executeTool(name: string, input: any): Promise<any> {
  switch (name) {
    ...
    case 'create_calendar_event': {
      ...
      if (error) return { error: error.message, payload }
    ...
    case 'browse_best_practices': {
      ...
      const folder = input.subfolder ? `${BP_ROOT}/${input.subfolder.replace(/^\/+/, '')}` : BP_ROOT
    ...
    case 'read_best_practices_file': {
      ...
      const extractResp = await anthropic.messages.create({
        ...
        messages: [{ role: 'user', content: [
          { type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: base64 } },
          { type: 'text', text: `You are analyzing a Best Practices document: "${name}". ${question} Include all key terms, market-standard benchmarks, acceptable ranges, red flags, and negotiating points. Be thorough.` },
        ]}],
      })
      const extracted = extractResp.content.filter((b: any) => b.type === 'text').map((b: any) => b.text).join('\n')
      return { file: name, path: input.path, content: extracted }
    ...
    case 'create_deal': {
      ...
      if (error) return { error: error.message }
      return { success: true, deal_id: data?.id, message: `Created deal ${input.company_name} in stage ${stage}. View it at /deals/${data?.id}` }
    }
    ...
    case 'update_deal_field': {
      ...
      if (input.field === 'stage') {
        if (value === 'Closed (Platform)' || value === 'Closed (Add-On)') extraUpdates.status = 'Closed'
        else if (String(value).startsWith('Pass')) extraUpdates.status = 'Dead'
        else extraUpdates.status = 'Active'
...
export function buildSystemPrompt(opts?: { channel?: 'web' | 'email'; senderName?: string }): string {
  const channel = opts?.channel ?? 'web'
  const base = `You are Nexus Assistant, an AI agent for Evolution Strategy Partners — a lower middle market PE firm in Austin/Dallas focused on infrastructure services ($3–7M EBITDA).
...
RULES:
1. For ANY counting question use count_deals — it returns the true total with no row cap
2. When counting deals by year, pass BOTH year and sourced_year and sum the union
3. When listing or searching deals, search ALL statuses — never default to Active only
4. For "deals from [banker/firm]" questions use the banker_firm parameter — it joins through contact_deal_links automatically
5. For ambiguous company names, search first and confirm the match before updating
6. Always fetch the deal/contact ID via a read tool before calling any write tool
7. For dollar amounts in tool calls, pass the number in millions as a plain string (e.g. "5.2" for $5.2M)
8. When presenting counts, show breakdowns
9. BEST PRACTICES — MANDATORY: For ANY question about credit agreements, financial covenants, DSCR, leverage, LOI terms, diligence checklists, or what is market-standard — call browse_best_practices first, then browse the relevant subfolder, then read_best_practices_file. Never answer these from training data alone.

DROPBOX: Root path "/Ken Nguyen/Evolution Strategy Partners". Deal files under ".../Deals/[Company]". Portfolio files under ".../Portfolio Co's/[Company]". Use list_portco_files / list_deal_files first (they resolve the linked path automatically). Best Practices library at "/Evolution Strategy Partners/Best Practices".
...
NEXUS NAVIGATION — mention these links when directing a user to a section: /dashboard, /pipeline, /deals, /intake, /contacts, /raises, /raises/contacts, /investors, /portfolio, /calendar, /notes, /best-practices.

Write tools always go to a confirmation step — never execute them without it. Format responses cleanly: bold for key figures, bullet points for lists, tables where helpful. Be concise.`
  }

  // Email channel
  return base + `

YOU ARE RESPONDING BY EMAIL${opts?.senderName ? ` to ${opts.senderName}` : ''}. There is NO interactive confirmation step over email:
- If the request asks you to CREATE or CHANGE something (create a deal, log a note, update a field, add a calendar event), perform the action directly using the write tools, then clearly state in your reply exactly what you did (include the deal/record name and a link like /deals/<id> when available).
- If a request is destructive or ambiguous (e.g. delete, or you can't identify which record), DO NOT guess — reply asking for clarification instead.
- Keep the reply concise and plain-text friendly: short paragraphs, simple dashes for bullets, no markdown tables or heavy formatting (it will render in an email client). Lead with the direct answer.
- Sign off as "— Nexus Assistant".`
}

// ─── Non-streaming agent loop (for email) ─────────────────────

/**
 * Runs the agent to completion without streaming and returns the final answer
 * text. Executes tool calls (including writes — there is no confirmation step
 * in this path, so only use it for trusted/internal channels).
 */
export async function runAgentToText(
  ...
  for (let i = 0; i < maxIter; i++) {
    const resp = await anthropic.messages.create({
      model: i === 0 ? AI_MODELS.balanced : AI_MODELS.fast,
      max_tokens: 2000,
      system,
      tools: TOOLS,
      messages,
    ...
    if (resp.stop_reason === 'end_turn' || toolUses.length === 0) {
      return { text: textBlocks || 'I could not find an answer to that.', toolsUsed }
    }
    ...
    const toolResults = await Promise.all(
      toolUses.map(async (t: any) => {
        ...
        try {
          ...
          return { type: 'tool_result' as const, tool_use_id: t.id, content: JSON.stringify({ error: err?.message || String(err) }) }
        }
      })
    )
